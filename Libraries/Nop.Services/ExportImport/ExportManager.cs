...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        //Short Excel Sheet
        private byte[] ExportOrderToExcelWithProducts(PropertyByName<Order>[] properties, IEnumerable<Order> itemsToExport)
        {
            var orderItemProperties = new[]
            {
                new PropertyByName<OrderItem>("ProductName", oi => oi.Product.Name),
                new PropertyByName<OrderItem>("Sku", oi => oi.Product.Sku),                
                new PropertyByName<OrderItem>("Quantity", oi => oi.Quantity),
                new PropertyByName<OrderItem>("DiscountExclTax", oi => oi.DiscountAmountExclTax),
                new PropertyByName<OrderItem>("ProductCost", oi => oi.Product.ProductCost),     
            };
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var xlPackage = new ExcelPackage(stream))
                {
                    ...
                    foreach (var order in itemsToExport)
                    {
                        ...
                        orderItemsManager.WriteCaption(worksheet, SetCaptionStyle, row, 5);
                        worksheet.Row(row).OutlineLevel = 1;
                        ...
                        foreach (var orederItem in orederItems)
                        {
                            row++;
                            orderItemsManager.CurrentObject = orederItem;
                            orderItemsManager.WriteToXlsx(worksheet, row, _catalogSettings.ExportImportUseDropdownlistsForAssociatedEntities, 5, fpWorksheet);
                            worksheet.Row(row).OutlineLevel = 1;
                            worksheet.Row(row).Collapsed = false;
                        }
        ...
        /// </summary>
        /// <param name="orders">Orders</param>
        public virtual byte[] ExportOrdersToExcel(IList<Order> orders)
        {
            ...
            //    foreach (var orderItem in orderItems)
            //    {
            //        productName.Add(orderItem.Product.Name);
            //    }
            //}
            ...
            var properties = new[]
            {
                new PropertyByName<Order>("OrderDate", p => p.CreatedOnUtc.ToShortDateString(), ignore),
                //new PropertyByName<Order>("ProductName", a => a.OrderItems.ToList().Select(b => b.Product.Name).ToList(), ignore),
                new PropertyByName<Order>("CustomerName", p => p.BillingAddress.Return(billingAddress => billingAddress.FirstName + "  " + billingAddress.LastName, String.Empty)),
                new PropertyByName<Order>("OrderSubtotal", p => p.OrderSubtotalInclTax, ignore),
                new PropertyByName<Order>("PaymentMethodAdditionalFee", p => p.PaymentMethodAdditionalFeeExclTax, ignore),
                new PropertyByName<Order>("OrderTotal", p => p.OrderTotal, ignore)
            };

            return _orderSettings.ExportWithProducts ? ExportOrderToExcelWithProducts(properties, orders) : ExportToXlsx(properties, orders);
        }
