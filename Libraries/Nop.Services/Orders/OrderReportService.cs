...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderReportService : IOrderReportService
    {
        ...
        public virtual IPagedList<AccountingDetailModel> newreport(int categoryId = 0, int manufacturerId = 0,
            ...
            bool showHidden = false)
        {
            ...
            var query1 = from orderItem in _orderItemRepository.Table
                         ...
                         //(! String.IsNullOrEmpty(orderItem.AttributeDescription) || orderItem.AttributeDescription.Contains("Primium"))&&
                         ...
                         select orderItem;
            IQueryable<AccountingDetailModel> query2 =
                //group by products
                from orderItem in query1
                where orderItem.AttributeDescription.Contains("Primium")
                group orderItem by orderItem.ProductId into g
                select new AccountingDetailModel
                {
                    ProductId = g.Key,
                    TotalAmount = g.Sum(x => x.PriceExclTax),
                    TotalQuantity = g.Sum(x => x.Quantity),
                };
            IQueryable<AccountingDetailModel> query3 =
               //group by products
               from orderItem in query1
               where orderItem.AttributeDescription.Contains("Standard")
               group orderItem by orderItem.ProductId into g
               select new AccountingDetailModel
               {
                   ProductId = g.Key,
            ...
            var result = new PagedList<AccountingDetailModel>(query2, pageIndex, pageSize);
            return result;
